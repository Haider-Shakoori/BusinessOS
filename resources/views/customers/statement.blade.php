...
@section('content')
    ...
    >
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-card sm:p-10 dark:border-gray-700 dark:bg-gray-800 print:rounded-none print:border-0 print:shadow-none print:p-0">
            {{-- Statement header --}}
            <div class="flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 pb-6 print:border-gray-300">
                <div>
                    <h2 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white print:text-gray-900">{{ $business?->name ?: config('app.name') }}</h2>
                    @if (! empty($businessDetails['address']) || ! empty($businessDetails['phone']) || ! empty($businessDetails['email']))
                        <address class="mt-1 not-italic text-sm text-gray-500 dark:text-gray-400 print:text-gray-600">
                ...
                <div class="text-start sm:text-end">
                    <p class="text-lg font-semibold text-gray-900 dark:text-white print:text-gray-900">{{ __('customers.statement.for') }}</p>
                    <p class="mt-1 text-sm font-medium text-gray-700 dark:text-gray-200 print:text-gray-800">{{ $customer->name }}</p>
                    <div class="mt-1 text-sm text-gray-500 dark:text-gray-400 print:text-gray-600">
            ...
            <div class="mt-6 flex flex-wrap items-center justify-between gap-2 text-sm">
                <p class="font-medium text-gray-700 dark:text-gray-200 print:text-gray-800">
                    {{ __('customers.statement.period') }}:
                    <span class="font-semibold">
                        {{ $dateFrom ?: __('customers.statement.all_dates') }} — {{ $dateTo ?: __('customers.statement.all_dates') }}
                    </span>
                    <span class="ms-2 font-medium text-gray-500 dark:text-gray-400 print:text-gray-600">({{ $baseCurrency }})</span>
                </p>
                <p class="text-gray-500 dark:text-gray-400 print:text-gray-600">
            ...
            <div class="mt-6 overflow-x-auto">
                <table class="w-full border-collapse text-sm">
                    <thead>
                        <tr class="border-b-2 border-gray-200 text-start text-xs font-semibold uppercase tracking-wide text-gray-500 dark:border-gray-300 dark:text-gray-400">
                        ...
                        @forelse ($rows as $row)
                            @if ($row['type'] === 'brought_forward')
                                <tr class="border-b border-gray-200 bg-gray-50 print:bg-gray-100">
                                    <td colspan="4" class="py-2 pe-3 font-semibold text-gray-700 print:text-gray-800">{{ __('customers.ledger.types.brought_forward') }}</td>
                            ...
                            @else
                                <tr class="border-b border-gray-200 print:border-gray-300 @if ($row['reversed']) opacity-60 dark:opacity-50 @endif">
                                    <td class="py-2 pe-3 whitespace-nowrap">{{ $row['date'] ?: '—' }}</td>
                                    <td class="py-2 pe-3 whitespace-nowrap">
                                        {{ __('customers.ledger.types.' . $row['type']) }}
                                        @if ($row['reversed'])
                                            <span class="ms-1 text-xs font-medium text-gray-500 dark:text-gray-400">({{ __('customers.ledger.reversed') }})</span>
                                        @endif
                                    </td>
                            ...
                            @endif
                        @empty
                            <tr class="border-b border-gray-200 print:border-gray-300">
                                <td colspan="7" class="py-6 text-center text-gray-500 dark:text-gray-400">{{ __('customers.ledger.no_transactions') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <dl class="mt-6 grid gap-2 sm:grid-cols-2 sm:gap-4">
                <div class="space-y-1 text-sm">
                    <div class="flex items-center justify-between gap-4">
                        <dt class="text-gray-500 dark:text-gray-400 print:text-gray-600">{{ $broughtForward !== null ? __('customers.statement.brought_forward') : __('customers.statement.opening') }}</dt>
                        <dd class="whitespace-nowrap font-medium tabular-nums" dir="ltr">{{ $broughtForward ?? $openingBalance }}</dd>
                    </div>
                    <div class="flex items-center justify-between gap-4">
                        <dt class="text-gray-500 dark:text-gray-400 print:text-gray-600">{{ __('customers.statement.debits_total') }}</dt>
                        <dd class="whitespace-nowrap font-medium tabular-nums" dir="ltr">{{ $periodDebits }}</dd>
                    </div>
                    <div class="flex items-center justify-between gap-4">
                        <dt class="text-gray-500 dark:text-gray-400 print:text-gray-600">{{ __('customers.statement.credits_total') }}</dt>
                ...
                <div class="flex items-end justify-end sm:text-end">
                    <div class="rounded-lg bg-gray-50 px-4 py-3 dark:bg-gray-700/50 print:bg-gray-100">
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 print:text-gray-600">{{ __('customers.statement.closing_balance') }}</dt>
                        <dd class="mt-0.5 text-2xl font-bold tabular-nums text-gray-900 dark:text-white print:text-gray-900" dir="ltr">{{ $closingBalance }}</dd>
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400 print:text-gray-600">{{ __('customers.statement.outstanding_balance') }}</p>
                    </div>
                </div>
            </dl>

            <p class="mt-8 border-t border-gray-200 pt-4 text-xs text-gray-400 dark:text-gray-500 print:border-gray-300 print:text-gray-500">
