                ...
                <x-ui.card>
                    <x-slot:header>
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('invoices.line_items') }}</h2>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                @foreach ($invoice->items as $item)
                                    <tr>
                                        <x-ui.td>
                                            <span class="text-gray-500 dark:text-gray-400">{{ $item->product?->name ?: __('invoices.no_product') }}</span>
                                        </x-ui.td>
                                        <x-ui.td>
...
<div class="flex items-center justify-between gap-4">
                                <dt class="text-base font-semibold text-gray-900 dark:text-white">{{ __('invoices.total') }}</dt>
                                <dd class="whitespace-nowrap text-base font-semibold tabular-nums text-gray-900 dark:text-white" dir="ltr">{{ $invoice->total }}</dd>
                            </div>
                            @if ($invoice->currency_code !== $baseCurrency)
                                <div class="flex items-center justify-between gap-4">
                                    ...
                                    <dd class="whitespace-nowrap text-xs tabular-nums text-gray-500 dark:text-gray-400" dir="ltr">{{ $invoice->base_amount }}</dd>
                                </div>
                                <p class="text-xs text-gray-400 dark:text-gray-500" dir="ltr">
                            ...
                            @if ($invoice->discount_type && $invoice->discount_amount > 0)
                                <div class="flex items-center justify-between gap-4">
                                    ...
                                    <dd class="whitespace-nowrap tabular-nums text-gray-900 dark:text-gray-100" dir="ltr">−{{ $invoice->discount_amount }}</dd>
                                </div>
                            @endif
                            @if ($invoice->tax_amount > 0)
                                <div class="flex items-center justify-between gap-4">
                                    <dt class="text-gray-500 dark:text-gray-400">{{ __('invoices.tax_amount') }}</dt>
                                    <dd class="whitespace-nowrap tabular-nums text-gray-900 dark:text-gray-100" dir="ltr">{{ $invoice->tax_amount }}</dd>
                                </div>
                            @endif
                            ...
                            <div class="flex items-center justify-between gap-4">
                                <dt class="text-gray-500 dark:text-gray-400">{{ __('payments.amount_paid') }}</dt>
                                <dd class="whitespace-nowrap tabular-nums text-emerald-600 dark:text-emerald-400" dir="ltr">{{ $invoice->amount_paid }}</dd>
                            </div>
                            @if ($invoice->amount_due > 0)
                                <div class="flex items-center justify-between gap-4">
                                    <dt class="text-gray-500 dark:text-gray-500">{{ __('payments.amount_due') }}</dt>
                                    <dd class="whitespace-nowrap tabular-nums text-gray-900 dark:text-gray-100" dir="ltr">{{ $invoice->amount_due }}</dd>
                                </div>
                            @endif
                        </dl>
                    </div>
                ...
                @if ($payments->isNotEmpty())
                    <div class="mt-6">
                        <x-ui.card>
                            <x-slot:header>
                                <div class="flex flex-wrap items-center justify-between gap-3">
                                    <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('payments.payment_history') }}</h2>
                                    <span class="text-sm text-gray-500 dark:text-gray-400">{{ trans_choice('payments.payment_count', $payments->count(), ['count' => $payments->count()]) }}</span>
                                </div>
                            </x-slot:header>
                                    ...
                                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                        @foreach ($payments as $payment)
                                            <tr>
                                                ...
                                                <x-ui.td class="text-end">
                                                    <span class="whitespace-nowrap font-medium tabular-nums text-gray-900 dark:text-white">{{ $payment->amount }}</span>
                                                </x-ui.td>
                                                <x-ui.td class="text-end">
                ...
                @if ($invoice->notes)
                    <div class="mt-6">
                        <x-ui.card>
                            <x-slot:header>
                                <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('invoices.notes') }}</h2>
                            </x-slot:header>
            ...
            <div class="lg:col-span-1">
                <x-ui.card>
                    <x-slot:header>
                        <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('invoices.information') }}</h2>
                    </x-slot:header>

                    <dl class="space-y-4">
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('invoices.number') }}</dt>
                            <dd class="mt-1 text-sm font-semibold text-gray-900 dark:text-white" dir="ltr">{{ $invoice->invoice_number }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('invoices.customer') }}</dt>
                        ...
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('invoices.date') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $invoice->date->format('Y-m-d') }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('currencies.currency') }}</dt>
                            <dd class="mt-1 text-sm font-semibold text-gray-900 dark:text-white" dir="ltr">{{ $invoice->currency_code }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('invoices.status') }}</dt>
                            <dd class="mt-1">
                                <x-ui.status-badge :status="$invoice->status->value" :label="__('invoices.statuses.'.$invoice->status->value)" />
                            </dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('invoices.source') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
                        ...
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('invoices.created_by') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $invoice->createdBy?->name ?: '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('invoices.created_at') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $invoice->created_at?->format('Y-m-d H:i') }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('invoices.updated_at') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $invoice->updated_at?->format('Y-m-d H:i') }}</dd>
                        </div>
                    </dl>
                </x-ui.card>
            </div>
