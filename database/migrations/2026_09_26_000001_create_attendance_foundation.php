...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (! Schema::hasTable('attendance_device_employees')) {
            Schema::create('attendance_device_employees', function (Blueprint $table) {
                ...
                $table->index(['business_id', 'employee_id'], 'att_business_employee_idx');
            });
        } else {
            $indexes = collect(Schema::getIndexes('attendance_device_employees'))
                ->pluck('name')
                ->filter()
                ->map(fn (string $name): string => strtolower($name));

            Schema::table('attendance_device_employees', function (Blueprint $table) use ($indexes) {
                if (! $indexes->contains('att_dev_user_unique')) {
                    $table->unique(['attendance_device_id', 'device_user_id'], 'att_dev_user_unique');
                }

                if (! $indexes->contains('att_dev_employee_unique')) {
                    $table->unique(['attendance_device_id', 'employee_id'], 'att_dev_employee_unique');
                }

                if (! $indexes->contains('att_business_employee_idx')) {
                    $table->index(['business_id', 'employee_id'], 'att_business_employee_idx');
                }
            });
        }
