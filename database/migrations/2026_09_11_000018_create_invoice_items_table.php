...
{
    /**
     * Invoice line items (Batch 15).
     *
     * Item rows belong exclusively to their parent invoice (invoice_id
     * cascadeOnDelete); there is NO business_id column and NO BelongsToBusiness
     * scope here — tenancy flows through the parent invoice row.
     *
     * product_id is nullable with nullOnDelete so soft-deleting (or even
     * hard-deleting) a product never destroys invoice history; the relation is
     * resolved with withTrashed() for display. The product's name at invoice
     * time is copied into `description`, so the item remains meaningful even
     * if the product is later removed. When the invoice was converted from a
     * quotation these snapshot values (description, quantity, unit_price,
     * tax_rate) are copied verbatim from the quotation items — never recomputed
     * from current Product/Tax master data (no-drift conversion).
     *
     * tax_id is a nullable reference to the business tax used at invoice time;
     * `tax_rate` is the 4-dp snapshot taken from the Tax row the moment the
     * invoice is written (never client-supplied), so later edits of the tax
     ...
     * tax) — the header subtotal/tax_amount/total are their aggregate. They are
     ...
     * historical snapshot attached to the trashed header.
     *
